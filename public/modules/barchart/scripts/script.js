...
(function() {
  ...
  d3.json('person-' + person + '.json', function(error, data) {
    var bar_svg, d, i, xAxis, _i, _len, _ref;
    if (error) {
    ...
    bar_svg.selectAll('g').data(data.channels).enter().append('g').attr('transform', 'translate(0,' + margin.top + ')').append('rect').attr('class', 'bar').attr('x', function(d, i) {
    ...
    bar_svg.selectAll('g').data(data.channels).append('image').attr('xlink:href', function(d) {
      return d.channelLogo;
    }).attr('height', 80).attr('width', 70).attr('x', function(d, i) {
      return x(d.channelName) + 10;
    }).attr('y', height);
    bar_svg.selectAll('g').append('rect').data(data.channels).attr('filter', 'url(#f1)').attr('class', 'tooltip shadow').attr('height', 45).attr('width', 76).attr('x', function(d, i) {
      return x(d.channelName) + 5;
    }).attr('y', function(d) {
      return y(d.channelCount) - 55;
    }).attr('rx', 20).attr('ry', 25);
    bar_svg.selectAll('g').append('rect').data(data.channels).attr('class', 'tooltip').attr('height', 45).attr('width', 76).attr('x', function(d, i) {
      return x(d.channelName) + 5;
    }).attr('y', function(d) {
      return y(d.channelCount) - 55;
    }).attr('rx', 20).attr('ry', 25);
    bar_svg.selectAll('g').append('text').data(data.channels).attr('text-anchor', 'middle').attr('class', 'tooltip name').attr('x', function(d, i) {
    ...
    bar_svg.selectAll('g').append('text').data(data.channels).attr('text-anchor', 'middle').attr('class', 'tooltip count').attr('x', function(d, i) {
      return x(d.channelName) + (x.rangeBand() / 2) - 3;
    }).attr('y', function(d) {
