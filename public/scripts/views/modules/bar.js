...
(function() {
  ...
  define(['jquery', 'underscore', 'backbone', 'd3', 'text!templates/modules/bar.html'], function($, _, Backbone, d3, tplBar) {
    ...
    return BarView = (function(_super) {
      var height, margin, width, x, y, yGrid;
      ...
      BarView.prototype.svg = function() {
        return d3.select(this.$el).append('svg').attr('id', 'barchart').attr('width', width).attr('height', height + margin.top + margin.bottom);
      };

      yGrid = function() {
        var yAxis;
        return yAxis = d3.svg.axis().scale(y).orient('left').ticks(4).tickSize(-width, 0, 0);
      };
      ...
      BarView.prototype.drawContent = function(data) {
        ...
        return bar_svg.selectAll('g.bar-g').data(data.channels).append('image').attr('xlink:href', function(d) {
      ...
      BarView.prototype.drawTooltip = function(data) {
        ...
        return bar_svg.selectAll('g.bar-g').append('text').data(data.channels).attr('text-anchor', 'middle').attr('class', 'tooltip count').attr('x', function(d, i) {
          return x(d.channelName) + (x.rangeBand() / 2) - 3;
        }).attr('y', function(d) {
